...
static void	sem_err_handler(int status, t_opcode code)
{
	...
	if (status == EAGAIN && code == LOCK)
		...
		exit_error("O_CREAT and O_EXCL were specified and the semaphore exists.");
	else if (status == EMFILE && code == INIT)
		exit_error("The process has already reached its limit for semaphores or file descriptors in use.");
	else if (status == ENFILE && code == INIT)
		exit_error("Too many semaphores or file descriptors are open on the system.");
	else if (status == ENOENT && (code == INIT || code == UNLINK))
		exit_error("O_CREAT is not set and the named semaphore does not exist.");
	else if (status == ENOSPC && code == INIT)
		exit_error("O_CREAT is specified, the file does not exist, and there is insufficient space available to create the semaphore.");
}
