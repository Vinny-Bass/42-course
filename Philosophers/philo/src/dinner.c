...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/17 16:59:28 by vsouza-v          #+#    #+#             */
/*   Updated: 2024/03/20 20:12:15 by vsouza-v         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	*dinner_monitor(void *data)
{
	...
	state = (t_state *)data;
	while (!all_threads_running(&state->table_mtx, &state->n_threads_running, \
	state->n_philos))
		;
	while (!simulation_finished(state))
	{
		...
		{
			is_eating = get_bool(&(state->philos + i)->philo_mutex, \
			&(state->philos + i)->eating);
...
void	start_dinner(t_state *state)
{
	...
	if (state->max_meals == 0)
		...
		safe_thread_handler(&state->philos[0].thread_id, single_dinner, \
		&state->philos[0], CREATE);
	else
		while (++i < state->n_philos)
			safe_thread_handler(&state->philos[i].thread_id, dinner_handler, \
			&state->philos[i], CREATE);
